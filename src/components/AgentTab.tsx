            ...
            {settings.perp_smart_sl_max_reentries === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {settings.perp_smart_sl_rebuy_mode === 'above_entry' && (
                  <>
                    <NumberInput label="Rebuy % venduto" help={'Quanta parte di ciò che è stato venduto ricomprare, quando il prezzo torna sopra l\'ingresso.'} value={settings.perp_smart_sl_rebuy_above_entry_pct} step={1} onChange={(perp_smart_sl_rebuy_above_entry_pct) => patch({ perp_smart_sl_rebuy_above_entry_pct })} />
                    <NumberInput label="R2 Split L1 %" help={'Quote di vendita del secondo giro, dopo un rebuy: quanta parte vendere al primo livello.'} value={settings.perp_smart_sl_split_l1_r2} step={0.01} onChange={(perp_smart_sl_split_l1_r2) => patch({ perp_smart_sl_split_l1_r2 })} />
                    <NumberInput label="R2 Split L2 %" help={'Quote del secondo giro: quanta parte vendere al secondo livello.'} value={settings.perp_smart_sl_split_l2_r2} step={0.01} onChange={(perp_smart_sl_split_l2_r2) => patch({ perp_smart_sl_split_l2_r2 })} />
                    <NumberInput label="R2 Split L3 %" help={'Quote del secondo giro: quanta parte lasciare fino allo stop.'} value={settings.perp_smart_sl_split_l3_r2} step={0.01} onChange={(perp_smart_sl_split_l3_r2) => patch({ perp_smart_sl_split_l3_r2 })} />
                  </>
                )}
                {settings.perp_smart_sl_rebuy_mode === 'delta' && (
                  <>
                    <NumberInput label="Delta L1" help={'Di quanto deve rimbalzare il prezzo per ricomprare il primo scaglione, come frazione della distanza verso lo stop.'} value={settings.perp_smart_sl_delta_l1} step={0.01} onChange={(perp_smart_sl_delta_l1) => patch({ perp_smart_sl_delta_l1 })} />
                    <NumberInput label="Delta L2" help={'Di quanto deve rimbalzare per ricomprare gli scaglioni successivi.'} value={settings.perp_smart_sl_delta_l2} step={0.01} onChange={(perp_smart_sl_delta_l2) => patch({ perp_smart_sl_delta_l2 })} />
                  </>
                )}
