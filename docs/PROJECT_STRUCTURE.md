...
| Shadow-stop: regola stop-stretto + reclaim confermato (NOTE/91) | Simula in tempo reale, su ogni posizione perp reale, la regola d'ingresso proposta da David — validata su 198 trade storici (V1+V2) prima dell'implementazione — senza mai toccare un ordine vero: stop a `perp_shadow_stop_buffer_pct` (default 0,1%) sotto il minimo/massimo della candela che ha generato il segnale; se colpito, attende una candela 5m INTERA oltre l'entry originale (conferma), poi un limit virtuale sull'entry (rientro); stessi TP1/TP2, fino a `perp_shadow_stop_max_reentries` rientri (default 1), orizzonte 24h (288 candele 5m). Macchina a stati pura in `agent/shadow_stop.py` (`advance()`, mai muta lo state ricevuto — stesso motivo di purezza di `breach.py`, NOTE/73); I/O (fetch candele, persistenza) in `agent/shadow_stop_runner.py`. Aggancio in apertura (`_handle_signal`, fire-and-forget come la telemetria NOTE/81) e avanzamento nello `slow_tick` (`advance_active_runs`), che continua a far girare un run anche dopo la chiusura reale della posizione: il confronto vero/simulato è lo scopo. Tabella nuova `shadow_stop_runs` (una riga per posizione, stato+eventi in JSON, esito e PnL virtuale denormalizzati). Diagnostica pura: nessun parametro esistente toccato, nessun ordine mai inviato. Test in `backend/tests/unit/test_shadow_stop.py` (motore) e `test_shadow_stop_runner.py` (I/O, candele iniettabili). |
